<?php
  if ( isset ( $_GET [ "o" ] ) && $_GET [ "o" ] == 1 ) {
    $ret_string = l2_additem2 ( $_GET [ "u" ], 0, $_GET [ "item_type" ], $_GET [ "amount" ], 0, 0, 0, 0, 0 );
    if ( $ret_string === "1" ) {
    ...
    if ( isset ( $_GET [ "o2" ] ) && $_GET [ "o2" ] == 1 ) {
      ...
      if ( $array_char ) {
        ...
        foreach ( $_GET [ "amount" ] as $item_id => $amount ) {
          $ret_string = l2_moveitem2 ( $_GET [ "u" ], $item_id, $new_char_id, str_replace ( " ", "", $amount ), 0 );
          if ( $ret_string === "1" ) {
...
<?php
  ...
  if ( isset ( $_GET [ "o" ] ) && ( $_GET [ "o" ] == 2 || $_GET [ "o" ] == 3 ) ) {
    $query = "select * from user_item (nolock) left outer join itemdata (nolock) on item_type=id where warehouse=0 and char_id=" . str_replace ( "'", "''", $_GET [ "u" ] ) . " and item_id in ( " . implode ( ", ", $_GET [ "id" ] ) . " ) order by name";
  } else {
    $query = "select * from user_item (nolock) left outer join itemdata (nolock) on item_type=id where warehouse=0 and char_id=" . str_replace ( "'", "''", $_GET [ "u" ] ) . " order by name";
  }
  $res = mssql_query ( $query, $link_world );
?>
<form method="get" action="<?php print ( $_SERVER [ "PHP_SELF" ] ); ?>">
...
<table class="center list" width="80%">
  <caption><?php print ( $_text [ 125 ] ); ?> [<?php print ( mssql_num_rows ( $res ) ); ?>]</caption>
  <tr class="naglowek">
...
?>
  <tr class="<?php print ( $i++ % 2 == 0 ? "even" : "odd" ); ?>">
    <td class="right"><a href="<?php print ( $_SERVER [ "PHP_SELF" ] ); ?>?p=5&amp;l=<?php print ( $_GET [ "l" ] ); ?>&amp;u=<?php print ( $_GET [ "u" ] ); ?>&amp;f=101&amp;item_id=<?php print ( $array_item [ "item_id" ] ); ?>"><?php print ( $array_item [ "item_id" ] ); ?></a></td>
...
<?php
  if ( isset ( $_GET [ "o" ] ) && ( $_GET [ "o" ] == 2 || $_GET [ "o" ] == 3 ) ) {
    $query = "select * from user_item (nolock) left outer join itemdata (nolock) on item_type=id where warehouse=1 and char_id=" . str_replace ( "'", "''", $_GET [ "u" ] ) . " and item_id in ( " . implode ( ", ", $_GET [ "id" ] ) . " ) order by name";
  } else {
    $query = "select * from user_item (nolock) left outer join itemdata (nolock) on item_type=id where warehouse=1 and char_id=" . str_replace ( "'", "''", $_GET [ "u" ] ) . " order by name";
  }
  $res = mssql_query ( $query, $link_world );
?>
<table class="center list" width="80%">
  <caption><?php print ( $_text [ 126 ] ); ?> [<?php print ( mssql_num_rows ( $res ) ); ?>]</caption>
  <tr class="naglowek">
    <th class="right" width="10%"><?php print ( $_text [ 99 ] ); ?></th>
    <th class="center" width="8%"><?php print ( $_text [ 100 ] ); ?></th>
    <th class="left" width="56%"><?php print ( $_text [ 101 ] ); ?></th>
