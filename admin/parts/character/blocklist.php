<?php
  $query = "select ud.*, p.*, unc.color_rgb from user_blocklist ub (nolock) inner join user_data ud (nolock) on ub.block_char_id=ud.char_id left outer join pledge p (nolock) on ud.pledge_id=p.pledge_id left outer join user_name_color unc (nolock) on ud.char_id=unc.char_id where ub.char_id=" . str_replace ( "'", "''", $_GET [ "u" ] ) . " order by block_char_name";
  $res = mssql_query ( $query, $link_world );
?>
<table class="center list" width="90%">
  <caption><?php print ( $_text [ 51 ] ); ?> [<?php print ( mssql_num_rows ( $res ) ); ?>]</caption>
  <tr class="naglowek">
