...
} elseif ( isset ( $_GET [ "pt" ] ) && ! empty ( $_GET [ "pt" ] ) ) {
  $query = "select us.*, ud.*, s.* from user_skill us (nolock) inner join user_data ud (nolock) on us.char_id=ud.char_id left outer join skilldata s (nolock) on us.skill_id=s.id and us.skill_lev=s.lev where us.skill_id=" . str_replace ( "'", "''", $_GET [ "skill_id" ] ) . " and us.subjob_id=" . str_replace ( "'", "''", $_GET [ "subjob_id" ] );
...
<table class="center list">
  <tr class="naglowek">
    <th width="25%"><?php print ( $_text [ 204 ] ); ?></th>
    <th width="25%"><?php print ( $array_clan [ "skill_id" ] ); ?></th>
    <th width="25%"><?php print ( $_text [ 205 ] ); ?></th>
